...
//	vector<string> files = GetFilesInDirectory(cameraPath);
//	for (int i = 0; i < files.size(); i++){
//		boost::property_tree::ptree pt;
//		boost::property_tree::ini_parser::read_ini(files[i], pt);
//		translateMatrix[i][0] = stof(pt.get<std::string>("posx"));
//		translateMatrix[i][1] = stof(pt.get<std::string>("posy"));
//		translateMatrix[i][2] = stof(pt.get<std::string>("posz"));
//		rotateMatrix[i][0] = stof(pt.get<std::string>("rotx"));
//		rotateMatrix[i][1] = stof(pt.get<std::string>("roty"));
//		rotateMatrix[i][2] = stof(pt.get<std::string>("rotz"));
//	}
//}
//
...
//
//			float z = depth;
//			if (z != 0 && std::abs(depth - bgdepth)> bgThreshold){
...
//
//
//pcl::texture_mapping::CameraVector loadCameras(pcl::PointCloud<PointXYZ>::Ptr res){
//	pcl::texture_mapping::CameraVector my_cams;
//	float theta = 0;
//	float thetaZ = 0;
//	int i = 1;
...
//		Eigen::Affine3f transform = Eigen::Affine3f::Identity();
//
//		transform(0, 3) = translateMatrix[i][0];
//		transform(1, 3) = translateMatrix[i][1];
//		transform(2, 3) = translateMatrix[i][2];
//
//		transform.rotate(Eigen::AngleAxisf(rotateMatrix[i][1] * M_PI / 180, Eigen::Vector3f::UnitY()));
//		transform.rotate(Eigen::AngleAxisf(rotateMatrix[i][2] * M_PI / 180, Eigen::Vector3f::UnitZ()));
//		transform.rotate(Eigen::AngleAxisf(rotateMatrix[i][0] * M_PI / 180, Eigen::Vector3f::UnitX()));
//
//		Eigen::Matrix4f mat = transform.matrix();
...
//	Eigen::Affine3f transformRotx = Eigen::Affine3f::Identity();
//	Eigen::Affine3f transformRoty = Eigen::Affine3f::Identity();
//	Eigen::Affine3f transformRotz = Eigen::Affine3f::Identity();
//
//	/*Eigen::Vector4f centroid;
//	pcl::compute3DCentroid(*a, centroid);
//	transformCenter(0, 3) =-centroid.x();
//	transformCenter(1, 3) =-centroid.y();
//	transformCenter(2, 3) = -centroid.z();*/
//
//	transform(0, 3) = translateMatrix[0][0];
//	transform(1, 3) = translateMatrix[0][1];
//	transform(2, 3) = translateMatrix[0][2];
//
//	transformRotz.rotate(Eigen::AngleAxisf(rotateMatrix[0][2] * M_PI / 180, Eigen::Vector3f::UnitZ()));
//	transformRotx.rotate(Eigen::AngleAxisf(rotateMatrix[0][0] * M_PI / 180, Eigen::Vector3f::UnitX()));
//	transformRoty.rotate(Eigen::AngleAxisf(rotateMatrix[0][1] * M_PI / 180, Eigen::Vector3f::UnitY()));
//
//
//	pcl::transformPointCloudWithNormals(*a, *a, transformRotx);
//	pcl::transformPointCloudWithNormals(*a, *a, transformRoty);
//	pcl::transformPointCloudWithNormals(*a, *a, transformRotz);
...
//		transformCenter = Eigen::Matrix4f::Identity();
//		transform = Eigen::Matrix4f::Identity();
//		transformRotx = Eigen::Affine3f::Identity();
//		transformRoty = Eigen::Affine3f::Identity();
//		transformRotz = Eigen::Affine3f::Identity();
//
//		PCL_INFO("Aligning %d\n", i);
...
//		transform(2, 3) = translateMatrix[i][2];
//
//		transformRotz.rotate(Eigen::AngleAxisf(rotateMatrix[i][2] * M_PI / 180, Eigen::Vector3f::UnitZ()));
//		transformRotx.rotate(Eigen::AngleAxisf(rotateMatrix[i][0] * M_PI / 180, Eigen::Vector3f::UnitX()));
//		transformRoty.rotate(Eigen::AngleAxisf(rotateMatrix[i][1] * M_PI / 180, Eigen::Vector3f::UnitY()));
//
//
//		pcl::transformPointCloudWithNormals(*b, *b, transformRotx);
//		pcl::transformPointCloudWithNormals(*b, *b, transformRoty);
//		pcl::transformPointCloudWithNormals(*b, *b, transformRotz);
...
//		
//		pairAlignFast(result,b,transf,transformMat,0.05,true);
//		showCloudsRight(transf, result);
//		corrections[i] = transformMat;
//		*b = *transf;
//		*result  += *b;
//		transf->clear();
//	}
...
//	pcl::texture_mapping::CameraVector my_cams = loadCameras(res);
